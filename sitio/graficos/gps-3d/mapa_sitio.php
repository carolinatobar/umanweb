...
$k 		=  ( $zmax - $zmin ) / ( $xmax - $xmin );

$a_z 	= ( 300 * ( $k + 1 ) ) / ( $zmax - $zmin );
$b_z 	= ( 300 * $k ) - ( $zmax * $a_z );


$lonFrom 	= deg2rad($xmin);
$lonTo		= deg2rad($xmax);
$lat 		= deg2rad($zmin);

$deltaLon 	= $lonTo - $lonFrom;

$angle = 2 * asin(sqrt(pow(sin(0 / 2), 2) + cos($lat) * cos($lat) * pow(sin($deltaLon / 2), 2)));

$earthRadius = 6371000;

//print ( $angle * $earthRadius );

$k_altura = 600 / ( $angle * $earthRadius );



function escaladoPuntos( $punto , $a , $b ) {
	$result = $punto * $a + $b;
	return $result;
}


//print escaladoPuntos($zmin,$a_z,$b_z);
...
<html lang="en">
	...
	<body>
		<script src="graficos/gps-3d/build/three.min.js"></script>
    	...
    	<script src="graficos/gps-3d/src/geometries/PlaneGeometry.js"></script>
    	<script type="text/javascript" src="graficos/gps-3d/examples/js/libs/dat.gui.min.js"></script>
		<script>


			var container, stats, plane, geometryxx, rotacion=0;
			var subida = true;
			var scale = 1;
			...
			function init() {
				...
				cube.position.z = 300;
				//scene.add( cube );
				//var plano = new THREE.PlaneGeometry(300, 300, 19, 19);
				//scene.add(plano);
				...
				foreach ( $lineas as $linea ) {
					...
					foreach ( $coords as $coord ) {
				    	...
				    	if( $x!=0 && $y!=0 && $z!=0 ) {
					    	...
					    	array_push($alturas, ( ( $y - $ymin ) * $k_altura ) );
					    	...
					    	$totaltodo++;
				    		$totalCoords++;
				    ?><?php print escaladoPuntos($x,$a_x,$b_x); ?>, <?php print ( $y - $ymin) * $k_altura; ?>, <?php print escaladoPuntos($z,$a_z,$b_z); ?>
				    <?php
				    	}
					}
				$totalLineas++;
...
<?php /*
			...
			function render() {
				...
				if(rotacion){
					scene.rotation.z = scene.rotation.z + 0.002;
				}
...
<script>
	...
	<?php
	print "Lineas: ".$totalLineas." Puntos: ".$totalCoords;
	print " Total: ".$totaltodo;
	?>