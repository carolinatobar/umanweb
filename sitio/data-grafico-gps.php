<?php 
 ...
 $maxContentHeight = $maxContentHeight > 0 ? $maxContentHeight : 330;
 ...
 if ( !isset( $_POST['n'] ) ) {
  ...
  $set = 0;
    ...
    $sql = "SELECT FECHA,X,Y,rapidez,ALTURA,direccion, DATE_FORMAT(FECHA, '%d/%m/%Y %H:%i:%s') AS FECHA2   
     FROM uman_gps 
     WHERE EQUIPO=$equipo AND X > '$n' AND X < '$sur' AND Y > '$e' AND Y < '$o' AND $fecha 
     ORDER BY FECHA ASC;";
    // echo "65: $sql \n";
    // $datos_gps 	= $db->query($sql); 
    $set = 1;
 ...
 if(!$set && isset($gps)) {
  ...
  for($i=0; $i<count($gps); $i++)
  {
    $tramo = array(
      ...
      'DH'=>0,
      'DV'=>0,
      'Exceso'=>($gps[$i]->rapidez > $maxvelocidad),
      'Fecha'=>$gps[$i]->FECHA2,
  ...
  // $link = 'doc-gen/velocidad-recorrido.php?equipo='.$equipo.'&fecha='.$rango.'&tipo=csv';
  $link = 'genera-documento.php?documento=velocidad_recorrido&equipo='.$equipo.'&fecha='.$rango.'&tipo=xls';
  ...
  foreach ($gps as $key => $g) {
    ...
    if(isset($gps[$key+1])){
      $d1 = new DateTime($gps[$key+1]->FECHA);
      $d2 = new DateTime($gps[$key]->FECHA);
      $dif = $d1->diff($d2);
      $min = ($dif->format("%h")*60)+($dif->format("%i"));
      if($min >= 10){
        $gData[] = array(strtotime($gps[$key]->FECHA)+10*60, null);
        $aData[] = array(strtotime($gps[$key]->FECHA)+10*60, null);
      }
    }
  }
?>
...
<style type="text/css">
  ...
  .gps-prev, .gps-next, .gps-play-pause, .gps-speed, .gps-scrollbar,
  .gps-first, .gps-last{
  ...
  .gps-first{ left: 10px; }
  .gps-prev{ left: 50px; }
  .gps-play-pause{ left: 90px; }
  .gps-next{ left: 130px; }
  .gps-last{ left: 170px; }
  .gps-speed{ left: 210px; }
  .gps-scrollbar{ left: 0px; }
  .gps-speed{
...
<div class="row">
 ...
 <div class="<?=Core::col(6,6,12,12)?>">
  <div id="map" style="width: 100%; height: <?=$maxContentHeight?>px;">
  </div>
  <!-- <div id="control">
    <div id="gps-first" class="btn gps-first fa fa-fast-backward"></div>
    <div id="gps-prev" class="btn gps-prev fa fa-step-backward"></div>
    <div id="gps-play-pause" class="btn gps-play-pause fa fa-play" data-action="play"></div>
    <div id="gps-next" class="btn gps-next fa fa-step-forward"></div>
    <div id="gps-last" class="btn gps-last fa fa-fast-forward"></div>
    <select id="gps-speed" class="gps-speed" onchange="changeSpeed(this);">
      <option value="1000" selected="selected">1x</option>
      <option value="750">1.5x</option>
      <option value="500">2x</option>
      <option value="250">3x</option>
    </select>
    <input type="range" id="gps-scrollbar" class="gps-scrollbar" min="0" max="<?=count($tramos)?>" step="1" value="0" />
  </div> -->
 </div>
</div>
...
<script type="text/javascript">
  ...
  var marker = [];
  var marker_bkp = [];
  var marker_ini = undefined;
  var marker_fin = undefined;
  ...
  <?php if($map_api == 'here'){ ?>
    ...
    var mapaCoords = <?=($tramos!=null)?json_encode($tramos):'null'?>;
  ...
  <?php if($map_api == 'google'){ ?>
    ...
    marker.push(new google.maps.Marker({
      ...
      // icon: {
      //   url: './assets/img/circulo_negro.png',
      //   scaledSize: new google.maps.Size(25, 25),
    ...
    var control_mapa = document.createElement('div');
    control_mapa.id='control_mapa';
    control_mapa.index=1;

    map.controls[google.maps.ControlPosition.LEFT_BOTTOM].push(control_mapa);

    $('<div id="control">')
      .append('<div id="gps-first" class="btn gps-first fa fa-fast-backward" onclick="go_first()";></div>')
      .append('<div id="gps-prev" class="btn gps-prev fa fa-step-backward" onclick=="go_prev();"></div>')
      .append('<div id="gps-play-pause" class="btn gps-play-pause fa fa-play" data-action="play" onclick="play_pause(this)"></div>')
      .append('<div id="gps-next" class="btn gps-next fa fa-step-forward" onclick="go_next();"></div>')
      .append('<div id="gps-last" class="btn gps-last fa fa-fast-forward" onclick="go_last();"></div>')
      .append(
        $('<select id="gps-speed" class="gps-speed" onchange="changeSpeed(this);">')
        .append('<option value="1000" selected="selected">1x</option>')
        .append('<option value="750">1.5x</option>')
        .append('<option value="500">2x</option>')
        .append('<option value="250">3x</option>')
      )
      .append('<input type="range" id="gps-scrollbar" class="gps-scrollbar" min="0" max="<?=count($tramos)?>" step="1" value="0" />')
      .appendTo(control_mapa);
  <?php } ?>
...
<script type="text/javascript">
  ...
  $(document).ready(function () {
    ...
    // var gps_scrollbar = document.querySelector("#gps-scrollbar");
    // gps_scrollbar.addEventListener("input", function() {
    ...
    $(window).on('resize', function(){
      ...
      $("#gps-scrollbar").css("width", "calc("+$("#map").css("width")+" - 252px)");
      // $("#control").css("top", (h-36)+"px");
    });
    $("#grafico").css("height", (h)+'px');
    // $("#control").css("height", (h-36)+"px");
    ...
    <?php if($map_api == 'google'){ ?>
      if(idx>=0){
        if(marker[idx]){
          ...
          let icon = marker[idx].getIcon();
          icon.strokeColor = "#F44336";
          icon.strokeWeight = 2;
          icon.fillColor = "#FF5722";
          marker[idx].setZIndex(9999);
          marker[idx].setIcon(icon);
        }

        if(marker[idx+1] && marker_bkp[idx+1]){
          console.log(marker[idx+1].label);
          marker[idx+1].setIcon(marker_bkp[idx+1]);
          marker[idx+1].setZIndex(marker_bkp[idx+1].zIndex);
        }
      }
    <?php } ?>
    $("#gps-scrollbar").val(idx);
  }
  ...
  function go_first(){
    <?php if($map_api == 'google'){ ?>
      if(idx>=0){
        if(marker_bkp[idx]){
          marker[idx].setIcon(marker_bkp[idx]);
          marker[idx].setZIndex(marker_bkp[idx].zIndex);
        }
      }
    <?php } ?>
    ...
    <?php if($map_api == 'google'){ ?>
      if(idx>=0){
        map.setCenter(marker_ini.position);          
      }
    <?php } ?>
    $("#gps-scrollbar").val(idx);
  }
  ...
  function go_last(){
    ...
    <?php if($map_api == 'google'){ ?>
      if(idx>=0){
        map.setCenter(marker_fin.position);
      }
    <?php } ?>
    $("#gps-scrollbar").val(idx);
  }

  function play_pause(o){
    let action = $(o).data("action");
    ...
    if(action=='play'){
      ...
      isPlaying = true;
      let speed = $("#gps-speed").val();
      intGPS = setInterval(playGPSTrip, speed);
    }else{
      $(o)
  ...
  function go_next(){
    idx++;
    if(idx>marker.length) idx = marker.length-1;
    <?php if($map_api == 'google'){ ?>
  ...
  function changeSpeed(o){
    if(isPlaying){
      let speed = $(o).val();
      clearInterval(intGPS);
      intGPS = setInterval(playGPSTrip, speed);
    }
  }
  function playGPSTrip(){
    idx++;
    if(idx>marker.length) idx = marker.length-1;
    <?php if($map_api == 'google'){ ?>
      if(idx<=marker.length){
