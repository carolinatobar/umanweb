...
<style media="screen">
  ...
  .box-aceite{
    background-image: url(assets/img/aceite.png); 
  ...
  <div class="row">
   <?php
     ...
     //    $enLinea['equipo'] = ($cuenta > 0)?TRUE:FALSE;
      ...
      //Coloca todos los neumáticos invisibles, de esta forma se pretende emular el funcionamiento final de la app
      $pintarAceite = false;
      for($ix=0; $ix<count($colores); $ix++){
        if($colores[$ix]['color'] == 'red') $pintarAceite = true;
        $colores[$ix] = ["color"=>'none', "dato"=>null, "fecha"=>null];
      }

      echo dibuja_caja_equipo($neumaticos, $colores , $enLinea['caja'] , $eqp_code, $pintarAceite);
      echo "</li>";
     }
 ...
 $(document).ready(function(){
   // $("#modal_equipo").modal();
    ...
    $(".btn_modal_equipo").click(function(){
      ...
      $("#modal_equipo_content").load("./modal/modal.equipo.aceite.php?equipo="+equipo);
    ...
    $("#modal_equipo").on('hidden.bs.modal', function () {
      // location.reload();
      $("#content").load('monitor.aceite.php');
    });    
  });
...
function dibuja_caja_equipo($neumaticos , $colores , $enLinea , $eqp_code, $pintarAceite=false){
 $neums = ''; $script = ''; $img = '';
 ...
 $aceite = $pintarAceite ? '<div class="box-aceite blink-alarmado"></div>' : '';
 $nombre = $eqp_code;
